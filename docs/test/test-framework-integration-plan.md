...
| 顺序 | 任务                     | 验收                                                                                                             |
| ---: | ------------------------ | ---------------------------------------------------------------------------------------------------------------- |
|    1 | 建立灰度环境设计         | 已落地：`docker-compose.gray.yml`、`.env.gray.example`、灰度 DB / Redis / 上传目录 / mock AI provider 与生产隔离 |
|    2 | 建立 seed / cleanup 设计 | 已落地：`pnpm gray:seed` / `pnpm gray:cleanup` 可重复初始化固定灰度用户、provider、模型                          |
|    3 | 建立全流程 E2E 设计      | 已落地：`antdXStudy/test/gray/release-smoke.gray.spec.ts` 覆盖聊天、文件、模型切换、错误、软删除和浏览器恢复     |
|    4 | 建立发布门禁设计         | 已落地：`.github/workflows/gray-release-gate.yml` 自动执行后端、前端与灰度 smoke，P0 / P1 失败阻断 workflow      |
|    5 | 建立巡检设计             | 已落地：灰度门禁支持 `workflow_dispatch` 与每日定时巡检                                                          |
