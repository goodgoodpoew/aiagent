...
- 已从 `AppModule` 删除全局 `CorsGuard` 注册。
- 已删除 `src/common/guard/cors.guard.ts`，避免继续维护第二套 CORS 响应头写入逻辑。
- `main.ts` 继续使用 `app.enableCors()` 作为唯一 CORS 策略入口。
- `configuration.ts` 新增 `cors.origins`，映射 `CORS_ORIGINS`，默认保留本地开发白名单。
- 生产环境如果配置 `CORS_ORIGINS=*` 会在启动时抛错，避免 wildcard 与 `credentials=true` 同时出现。
- `.env.example` 增加 `CORS_ORIGINS` 示例。

已跑验证：

- `rg -n "CorsGuard|common/guard/cors.guard|Access-Control-Allow-Origin" ai-proxy-server/src ai-proxy-server/test` 无残留匹配。
- `pnpm exec eslint src/main.ts src/app.module.ts src/config/configuration.ts` 通过。
- `pnpm build`（后端）通过。
- `bash scripts/check-generated-artifacts.sh` 通过。

未验证事项：

- 尚未启动浏览器实测本地前端、SSE、文件上传的 CORS 行为。
...
- `configuration.ts` 新增 `files.uploadRoot`，映射 `UPLOAD_ROOT`，默认 `uploads`。
- `LocalFileStorage` 注入 `ConfigService`，使用 `files.uploadRoot` 初始化本地存储根目录。
- 文件保存、读取、删除统一经过 `resolveStoragePath()`，使用 `path.resolve()` + `path.relative()` 校验最终路径必须位于 upload root 内。
- `.env.example` 增加 `UPLOAD_ROOT=uploads` 示例。
- 新增 `local-file.storage.spec.ts`，覆盖配置 upload root 生效和路径逃逸拒绝。

已跑验证：

- `pnpm test:unit -- local-file.storage` 通过。
- `pnpm exec eslint src/files/storage/local-file.storage.ts src/files/storage/local-file.storage.spec.ts src/config/configuration.ts` 通过。
- `pnpm test:unit`（后端）通过，21 个 suites / 72 个 tests。
- `pnpm build`（后端）通过。
- `bash scripts/check-generated-artifacts.sh` 通过。

未验证事项：

- 尚未启动集成测试环境验证真实上传文件写入 `uploads-test`。
- 灰度清理脚本已继续使用 `UPLOAD_ROOT` 环境变量，但未在灰度容器中实跑。
...
- 新增 `.github/workflows/frontend-tests.yml`。
- PR 与 `main` 分支 push 在命中 `antdXStudy/**`、前端 workflow 或生成产物检查脚本变更时自动触发。
- workflow 使用 Node.js 20 与 pnpm 10.8.0，缓存依赖键指向 `antdXStudy/pnpm-lock.yaml`。
- 快速门包含 `pnpm build`、`pnpm test:unit`、`pnpm test:components`。
- 前端门禁前置执行 `bash scripts/check-generated-artifacts.sh`，避免 coverage、Playwright 报告、Umi 产物回流。
- Playwright e2e 仍保留在 gray release gate，避免普通 PR 门禁过慢。

已跑验证：

- `bash scripts/check-generated-artifacts.sh` 通过。
- `pnpm test:unit`（前端）通过。
- `pnpm test:components`（前端）通过。
- `pnpm build`（前端）通过。

未验证事项：

- 尚未在 GitHub Actions 真实 PR 环境中执行新增 `frontend-tests.yml`。
...
执行状态：部分完成（2026-06-08）。

实际落地：

- 后端内部文件读取工具的附件快照补充 `tokenEstimate`，用户消息 metadata 能记录可读附件 token 估算。
- 前端用户消息附件区会展示“已读取”“未进入上下文”、token 估算和不可用原因。
- 保留 assistant 过程面板中已有 `file_read` 展示，不改变 SSE v2 协议和消息结构。

已跑验证：

- `pnpm test:unit -- builtin-tool.adapter native-agent-engine.service`（后端）通过。
- `pnpm test:components -- MessagePartsRenderer.component.spec.tsx`（前端）通过，实际执行 `src/pages` 下 5 个组件测试文件 / 17 个测试。
- `pnpm build`（前端）通过。
- `pnpm build`（后端）通过。
- 浏览器打开 `http://localhost:8000` 可加载应用并按未登录状态跳转 `/login`。

未验证事项：

- 尚未构造真实附件消息做浏览器气泡视觉检查。
- 尚未覆盖大文件内容级截断原因展示；当前只展示可读/不可用与 token 估算。
...
- 已新增 `ProviderModelCapabilities` 稳定能力声明，覆盖 `chat`、`stream`、`toolCalling`、`reasoning`、`vision`、`jsonMode`。
- 已新增 `ProviderCapabilityService`，从数组或对象形态的 `features` 归一化模型能力，并对未知 provider 保持保守默认。
- `ModelProviderService` 的列表、详情和模型 CRUD 响应会保留原始 `features`，同时返回派生 `capabilities`。
- `ModelProviderRegistryService.resolveChatProvider()` 已从模型能力填充 `reasoning` 与 `toolCalling`，不再只靠 provider 名称硬编码。
- 前端 `ProviderModel` 类型已透出 `capabilities`，后续 UI 可基于能力声明禁用 tools/reasoning 等选项。
- 种子模型补充基础能力标签，未新增数据库迁移。

已跑验证：

- `pnpm test:unit -- provider-capability model-provider-registry`（后端）通过。
- `pnpm test:unit -- src/service/platform.spec.ts`（前端）通过，实际覆盖 service/store 现有 109 个测试。
- `pnpm build`（后端）通过。

未验证事项：

- 尚未启动真实后端手工验证 `/api/model-providers` 响应体。
- 尚未实现前端基于 `capabilities` 的 UI 禁用或提示。
- 尚未实现 Anthropic/Gemini 原生 adapter，本步只完成 Provider Adapter 前置能力声明基线。
